<template>
    <div class="con" style="position:relative;">
       <div class="con1" style="margin-left:140px;background-color:white;width:60%">
           <div style="width:20px"></div>
           <div>选出导课周次:</div>
            <div style=" margin-left: 30px;display:flex;">
                <div>
                    <input id="start" v-model="start_week"  type="text" list="downlist_one" placeholder="选择起始周次" style="width:200px; height:35px;border-radius:3px;border:1px solid rgb(196,200,207);padding-left:10px;dispaly:inline;" >
                    <datalist id="downlist_one">
　　                    <option  v-for="index of zhoushu" :key="index">{{index}}</option>
                    </datalist>
                </div>
                <div style="display:inline">至</div>
                <div>
                    <input id="end" v-model="end_week"  type="text" list="downlist_two"  placeholder="选择截至周次" style="width:200px; height:35px;border-radius:3px;border:1px solid rgb(196,200,207);padding-left:10px;" >
                    <datalist id="downlist_two">
　　                    <option  v-for="index of zhoushu" :key="index">{{index}}</option>
                    </datalist>
                </div>
           </div>
           <div style=" margin-left: 30px;">
                <el-button type="primary" style="width:100px;height:40px">全部周次</el-button>
           </div>
       </div>
       <div class="con2" style="margin-left:140px;background-color:white;width:60%">
           <div style="width:20px"></div>
           <div>导出课表：</div>
           <el-switch
            v-model="value1"
       ...
       <div class="con3" style="margin-left:140px;background-color:white;width:60%">
           <div>导出调代课历史记录：</div>
           <el-switch
            v-model="value2"
            active-color="#13ce66"
...
            },
            ...
            // //获取截至周次
            //  get_end_week(){
            //     const end_week=document.getElementById("end").value
            //     console.log(start_week)
            // },
            down_timetable(){
                const start_week=document.getElementById("start").value
                console.log(start_week)
                const end_week=document.getElementById("end").value
                ...
                this.$http.post('http://47.97.193.46:8001/files/download/courses/',
                {
    ...
                    // var blob = new Blob([response], { type: responseType }); // ArrayBuffer , mime类型
