<template>
    <div class="con" style="position:relative;">
       <div class="con1" style="background-color:white;width:60%">
       ...
       <div style="background-color:white;width:60%;margin-left: 10%;"><el-button type="success"  @click="datasetting"  style="margin-left:70%;width: 100px;height: 50px;background-color: rgb(133,159,62);">确认</el-button></div>
...
            },
            datasetting(){
                ...
                const user_id=window.sessionStorage.getItem("user_id")
                 this.$http.put('http://47.97.193.46:8001/schools/firstweek/'+user_id+'/',
                    {
                        date:this.$data.value3
                    },
                    {
                        headers:{
                        ...
                        if(res.status==200){
                            this.$message.success("修改学期日期成功")
                        }
                        else{
                            this.$message.error("修改学期日期失败")
                        }
                    }).catch(err=>{
                        this.$message.error("修改学期日期失败,请填写日期")
                    })
            },
        },
    }
